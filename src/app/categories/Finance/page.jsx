...
export default function BlogPage() {
  ...
  const categoriesToDisplay = ["Finance & Banking" ,"Insurance","Business & Entrepreneurship","Finance & Money Management","Health & Wellness"]; // Add the categories you want to display here

  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch("https://jsonserver1-yblk.onrender.com/posts", { cache: 'no-store' });
        if (!response.ok) {
